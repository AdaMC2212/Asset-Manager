...
import { addMoneyTransaction, syncAutoDebitRuleFromTransaction, updateMoneyTransaction } from '../../app/actions';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (initialData && initialData.rowIndex) {
        ...
        if (result.success && initialData.isAutoGenerated && initialData.autoRuleId) {
          const updateFutureRule = confirm('Apply these edits to future auto-debit months too? Click OK for future months, or Cancel for this occurrence only.');
          if (updateFutureRule) {
            const ruleResult = await syncAutoDebitRuleFromTransaction(initialData.autoRuleId, payload);
            if (!ruleResult.success) {
              alert('Transaction updated, but future auto-debit rule was not updated.');
            }
          }
        }
      } else {
